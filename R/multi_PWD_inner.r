...
#' @name multi_PWD_inner
#'
#' @description
#' This is primarily a workhorse routine that `multi_PWD` calls and, given a
...
#'
#'   \item{alpha}{the vector of *I* estimated intercepts \eqn{\alpha_i}.}
#'   \item{beta}{the vector of *I* estimated slopes \eqn{\beta_i}.}
...
#'   \item{rho}{the input value of \eqn{\rho}.}
...
multi_PWD_inner <- function(X, rho, refine=1, eps=1e-9)  {
  size   <- dim(X)
  n      <- size[1]
  I      <- size[2]
  nI     <- n*I
  ...
  return(list(alpha=unname(alpha), beta=unname(beta), fity=fity,
              mu=mu, resi=resi,
              rho=rho, sigma=rho*kappa, kappa=kappa,
              lambda=lambda,
              L=L, scalr=scalr))
}
